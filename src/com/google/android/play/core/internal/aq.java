...
public final class aq<T extends IInterface> {
    ...
    private static final Map<String, Handler> f161a = new HashMap();
    ...
    public final void r(ah ahVar) {
        Handler handler;
        synchronized (f161a) {
            if (!f161a.containsKey(this.d)) {
                HandlerThread handlerThread = new HandlerThread(this.d, 10);
                handlerThread.start();
                f161a.put(this.d, new Handler(handlerThread.getLooper()));
            }
            handler = f161a.get(this.d);
        }
        handler.post(ahVar);
    }
