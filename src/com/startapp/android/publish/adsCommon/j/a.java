...
public class a {
    ...
    private HashMap<Integer, SensorEvent> f500a = new HashMap<>();

    public int a(SensorEvent sensorEvent) {
        int size;
        synchronized (this) {
            int type = sensorEvent.sensor.getType();
            SensorEvent sensorEvent2 = this.f500a.get(Integer.valueOf(type));
            if (sensorEvent2 == null || sensorEvent2.accuracy <= sensorEvent.accuracy) {
                this.f500a.put(Integer.valueOf(type), sensorEvent);
            }
            size = this.f500a.size();
        }
        return size;
    }
    ...
    public JSONArray a() {
        ...
        for (SensorEvent next : this.f500a.values()) {
